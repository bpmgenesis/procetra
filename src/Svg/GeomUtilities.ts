import { CGRectangle, CGPoint, CGSize, CGAffineTransform, Matrix } from '@tuval/cg';
import { float, Out, New } from '@tuval/core';
...
export class GeomUtilities {
    ...
    // from the array of CGPoint's only the first three will be used.
    ...
    public static GetNearestIntersectionPoint(rect: CGRectangle, p1: CGPoint, p2: CGPoint, result: Out<CGPoint>): boolean {
        ...
        let pointF5: Out<CGPoint> = New.Out();
    ...
    public static GetSurroundingRectangle(angleInDeg: float, rect: CGRectangle): CGPoint[] {
        ...
        if (angleInDeg <= 90) {
            var point1 = new CGPoint(rect.Left + x1, rect.Top - y1);
            var point2 = new CGPoint(rect.Left - x2, rect.Top + y2);
            var point3 = new CGPoint(rect.Left + x3, rect.Bottom + y3);
            var point4 = new CGPoint(rect.Right + x4, rect.Bottom - y4);
        } else if (angleInDeg > 90 && angleInDeg <= 180) {
            var point2 = new CGPoint(rect.Left + x1, rect.Top - y1);
            var point3 = new CGPoint(rect.Left - x2, rect.Top + y2);
            var point4 = new CGPoint(rect.Left + x3, rect.Bottom + y3);
            var point1 = new CGPoint(rect.Right + x4, rect.Bottom - y4);
        } else if (angleInDeg > 180 && angleInDeg <= 270) {
            var point3 = new CGPoint(rect.Left + x1, rect.Top - y1);
            var point4 = new CGPoint(rect.Left - x2, rect.Top + y2);
            var point1 = new CGPoint(rect.Left + x3, rect.Bottom + y3);
            var point2 = new CGPoint(rect.Right + x4, rect.Bottom - y4);
        } else if (angleInDeg <= 360) {
            var point4 = new CGPoint(rect.Left + x1, rect.Top - y1);
            var point1 = new CGPoint(rect.Left - x2, rect.Top + y2);
            var point2 = new CGPoint(rect.Left + x3, rect.Bottom + y3);
            var point3 = new CGPoint(rect.Right + x4, rect.Bottom - y4);
        }
