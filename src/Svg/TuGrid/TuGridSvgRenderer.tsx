...
export class TuGridSvgRenderer {
            ...
            if (int32 > 0) {
                ...
                for (let i = pointF.X; i < pointF1.X; i = i + width) {
                    ...
                    if (view.DocScale > (grid as any).myPaintMinorScale || int321 == 0) {
                        TuShape.DrawSvgLine(result, view, pen1, i, pointF.Y, i, pointF1.Y);
                    }
                    int321++;
                    if (int321 >= int32) {
                        int321 = 0;
                ...
                for (let j = pointF.X; j < pointF1.X; j = j + width) {
                    TuShape.DrawSvgLine(result, view, pen, j, pointF.Y, j, pointF1.Y);
                }
            }
        }
            ...
            if (height1 > 0) {
                ...
                for (let k = pointF.Y; k < pointF1.Y; k = k + height) {
                    ...
                    if (view.DocScale > (grid as any).myPaintMinorScale || int322 == 0) {
                        TuShape.DrawSvgLine(result, view, pen2, pointF.X, k, pointF1.X, k);
                    }
                    int322++;
                    if (int322 >= height1) {
                        int322 = 0;
                ...
                for (let l = pointF.Y; l < pointF1.Y; l = l + height) {
                    TuShape.DrawSvgLine(result, view, pen, pointF.X, l, pointF1.X, l);
                }
            }
        }
        pen.Dispose();
            ...
            for (let i = pointF.Y; i < pointF1.Y; i = i + height) {
                for (let j = pointF.X; j < pointF1.X; j = j + width) {
                    ...
                    if (!color.Equals(CGColor.Empty)) {
                        ...
                        TuShape.DrawSvgRectangle(result, view, undefined, solidBrush, j, i, width, height);
                    }
                    int321++;
    ...
    public static Apply(view:TuView,grid: TuGrid): any[] {
            ...
            if (shadowOffset.Width !== 0 || shadowOffset.Height !== 0) {
                ...
                if (width > 0) {
                    if (height > 0) {
                        ...
                        TuShape.DrawSvgPolygon(result, view, undefined, shadowBrush, pointFArray);
                        view.freeTempPointArray(pointFArray);
                    }
                    else if (height >= 0) {
                        TuShape.DrawSvgRectangle(result, view, null, shadowBrush, pointF1.X, pointF1.Y, width, rectangleF1.Height);
                    }
                    else {
                        ...
                        TuShape.DrawSvgPolygon(result, view, undefined, shadowBrush, pointFArray1);
                    ...
                    if (height > 0) {
                        ...
                        TuShape.DrawSvgPolygon(result, view, undefined, shadowBrush, pointFArray2);
                        view.freeTempPointArray(pointFArray2);
                    }
                    else if (height >= 0) {
                        TuShape.DrawSvgRectangle(result, view, undefined, shadowBrush, pointF4.X, pointF4.Y, -width, rectangleF1.Height);
                    }
                    else {
                        ...
                        TuShape.DrawSvgPolygon(result, view, undefined, shadowBrush, pointFArray3);
                        view.freeTempPointArray(pointFArray3);
                    }
                }
                else if (height > 0) {
                    TuShape.DrawSvgRectangle(result, view, undefined, shadowBrush, pointF3.X, pointF3.Y, rectangleF1.Width, height);
                }
                else if (height < 0) {
                    TuShape.DrawSvgRectangle(result, view, undefined, shadowBrush, pointF4.X, pointF4.Y, rectangleF1.Width, -height);
                }
            }
        }
        if (brush != null) {
        ...
        if (grid.Pen != null) {
            TuShape.DrawSvgRectangle(result, view, grid.Pen, undefined, rectangleF1.X, rectangleF1.Y, rectangleF1.Width, rectangleF1.Height);
        }
        return result;
    }
}