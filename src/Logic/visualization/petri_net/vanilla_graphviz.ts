export class PetriNetVanillaVisualizer {
	...
	static apply(acceptingPetriNet, debug=false) {
		...
		for (let transKey in petriNet.transitions) {
			...
			let nUid = PetriNetVanillaVisualizer.nodeUuid();
			if (trans.label != null) {
				ret.push(nUid+" [shape=box, label=\""+trans.label+"\", fontname=\"Ubuntu\"  fontsize=\"10\", color=\"orangered3\", style=filled, fillcolor=\"peachpuff2\" ]");
			}
			else {
				if (debug == true) {
					...
					ret.push(nUid+" [shape=box, label=\" \", style=filled, fillcolor=coral2, height=1, width= 0.1]");
				}
			}
			uidMap[trans] = nUid;
		}
