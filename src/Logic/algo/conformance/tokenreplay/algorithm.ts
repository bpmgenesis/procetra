export class TokenBasedReplayResult {
    ...
    transExecutions: {};
    arcExecutions: {};
    totalConsumedPerPlace: {};
    totalProducedPerPlace: {};
    totalMissingPerPlace: {};
    totalRemainingPerPlace: {};
    totalTraces: any;
    fitTraces: number;
    logFitness: number;
...
export class TokenBasedReplay {
    ...
    static applyListListAct(listListActivities, acceptingPetriNet, reachFm = true, retMarking = false) {
        ...
        for (let activities of listListActivities) {
            let tbrResult: any = TokenBasedReplay.performTbr(activities.split(","), transitionsMap, acceptingPetriNet, invisibleChain, reachFm);
            if (retMarking) {
