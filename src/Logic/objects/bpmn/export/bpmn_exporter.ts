export class BpmnExporter {
    static DUMMY_SEP = "AIOEWFRIUOERWQIO";
    static uuidv4() {
...
static apply(bpmnGraph) {
    ...
    for (let nodeId in bpmnGraph.nodes) {
        ...
        let bounds: any = document.createElementNS("", "omgdc"+BpmnExporter.DUMMY_SEP+"Bounds");
    ...
    for (let edgeId in bpmnGraph.edges) {
        ...
        if (Object.keys(edge.waypoints).length > 0) {
            ...
            // layouting has not been done. exports with default
                let xmlWaypoint: any = document.createElementNS("", "omgdi"+BpmnExporter.DUMMY_SEP+"waypoint");
