...
export const Utils = {
    createPaper: function (container, props) {
        ...
        const paper = TuvalSvg(container, width, height);
        if (props.viewbox) {
    ...
    updateElementProps: function (element, props) {
        if (element) {
            // fix matrix bug
            element.matrix = TuvalSvg.matrix();
                ...
                switch (key) {
                    ...
                    case "onAnimation": {
                        if (typeof props[key] === "function") {
                            element.onAnimation(props.onAnimation);
                        }
                        break;
                    }
                    case "animateWith": {
                    ...
                    case "blur": {
                        if (typeof props[key] === "number" && props.blur) { element.blur(props.blur); }
                        break;
                    }
                    case "touchcancel": {
            ...
            // fix raphael #491
            if (TuvalSvg.svg && element.node && element.node.nodeName === "text" && element.node.childNodes.length > 0) {
                setTimeout(function () {
                    if (element.node) {
