...
var ORIENTATION_THRESHOLD = {
  ...
  export function getExpandedBounds(element) {
    var bounds = {
  ...
  export function connectRectangles(source, target, preferredLayout = 'h:h') {
    ...
    if (directions === 'h:h') {
      ...
      case 'right':
      case 'bottom-right':
        start = { original: start, x: source.x, y: start.y } as any;
        end = { original: end, x: target.x + target.width, y: end.y } as any;
        break;
      case 'top-left':
      case 'left':
      case 'bottom-left':
        start = { original: start, x: source.x + source.width, y: start.y } as any;
        end = { original: end, x: target.x, y: end.y } as any;
        break;
      }
      ...
      case 'top':
      case 'top-right':
        start = { original: start, x: start.x, y: source.y + source.height } as any;
        end = { original: end, x: end.x, y: target.y } as any;
        break;
      case 'bottom-left':
      case 'bottom':
      case 'bottom-right':
        start = { original: start, x: start.x, y: source.y } as any;
        end = { original: end, x: end.x, y: target.y + target.height } as any;
        break;
      }
  ...
  export function getDirections(orientation, defaultLayout) {
    switch (orientation) {
    case 'intersect':
      return null;
    case 'top':
  ...
  export function is(type, expected) {
