...
export class MeanChart extends TvChart {
    overTotal: number;
    public AtDataBind: Lazy<any>;
    public override InitComponents() {
        ...
        this.TooltipRender.add((args: ITooltipRenderEventArgs) => {
            if (args.series.name === 'Mean') {
                args.text = `${args.data.pointX}<br>Mean duration: ${humanizeDuration(Convert.ToInt32(args.data.pointY as any) * 1000, { units: ["d", "h", "m", "s"], round: true })}`;
            } else {
    ...
    public SetChartData(chartData: any[]) {
        chartData = sort(chartData, ['mean'], true);
        const data = []
        const data1 = [];
        let total = 0;
        this.overTotal = 0;
        for (let i = 0; i < chartData.length; i++) {
            this.overTotal += chartData[i]['mean'];
        }
        for (let i = 0; i < chartData.length; i++) {
            total += chartData[i]['mean'];
            data.push({
    ...
    /*     public LazyBind() {
            StateService.GetCurrentActivityInfo().then(data => this.SetChartData(data));
