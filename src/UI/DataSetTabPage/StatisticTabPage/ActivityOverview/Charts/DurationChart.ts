...
export class DurationChart extends TvChart {
    overTotal: number;
    public override InitComponents() {
        ...
        this.TooltipRender.add((args: ITooltipRenderEventArgs) => {
            if (args.series.name === 'Mean') {
                args.text = `${args.data.pointX}<br>Duration range: ${humanizeDuration(Convert.ToInt32(args.data.pointY as any) * 1000, { units: ["d", "h", "m", "s"], round: true })}`;
            } else {
    ...
    public SetChartData(chartData: any[]) {
        chartData = sort(chartData, ['duration_range'], true);
        const data = []
        const data1 = [];
        let total = 0;
        this.overTotal = 0;
        for (let i = 0; i < chartData.length; i++) {
            this.overTotal += chartData[i]['duration_range'];
        }
        for (let i = 0; i < chartData.length; i++) {
            total += chartData[i]['duration_range'];
            data.push({
                x: chartData[i]['concept:name'],
                y: chartData[i]['duration_range'],
            });
            data1.push({
                x: chartData[i]['concept:name'],
                y: total,
