...
export class Lexer {
    ...
    private _fetchNextToken() {
        ...
        do {
            ...
            ch = this.stream.next();
            if (ch === -1) {
                return new Token(null, TokenTypes.EOF);
            }
        ...
        if (token !== null && token.isSymbol("(*")) {
            ...
            token = new Token(value, TokenTypes.COMMENT);
        }
        if (token === null && utils.isIdentifierStart(ch)) {
            ...
            var tokenType = isReservedWord(value) ? TokenTypes.RESERVED_WORD : TokenTypes.IDENTIFIER;
        ...
        if (token === null && (utils.isDigit(ch) || ch === ".")) {
            if (ch === ".") {
                ...
                if (nextCh === ".") {
                    // Two dots.
                    this.stream.next();
                    token = new Token("..", TokenTypes.SYMBOL);
            ...
            if (token === null) {
                ...
                while (true) {
                    ...
                    if (ch === "." && !sawExp) {
                        ...
                        if (ch === '+' || ch === '-' || utils.isDigit(ch)) {
                            ...
                            // Not allowed after "e".
                            token = new Token(value + ch, TokenTypes.NUMBER);
                ...
                token = new Token(value, TokenTypes.NUMBER);
            }
        }
        if (token === null && ch === "{") {
            // Comment.
        ...
        if (token === null && ch === "'") {
            ...
            token = new Token(value, TokenTypes.STRING);
        }
        if (token === null) {
            // Unknown token.
            token = new Token(ch, TokenTypes.SYMBOL);
            token.lineNumber = lineNumber;
    ...
    private _pickLongestToken(ch, symbols) {
        ...
        return new Token(longestSymbol, TokenTypes.SYMBOL);
    }
}
