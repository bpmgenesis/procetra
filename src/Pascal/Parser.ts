...
export class Parser {
    lexer: any;
    public constructor(lexer: any) {
        this.lexer = lexer;
    }

    // Parse an entire Pascal program.
    public parse(symbolTable) {
        var node = this._parseSubprogramDeclaration(symbolTable, NodeTypes.PROGRAM);
    ...
    private _moreToCome(separator, terminator) {
    ...
    private _expectReservedWord(reservedWord, message?) {
    ...
    private _expectSymbol(symbol, message?) {
        var token = this.lexer.next();
        if (token.tokenType !== TokenTypes.SYMBOL || token.value !== symbol) {
    ...
    private _expectIdentifier(message) {
        var token = this.lexer.next();
    ...
    private _parseDeclarations(symbolTable) {
        var declarations = [];
    ...
    private _parseDeclaration(symbolTable) {
        var token = this.lexer.peek();
        ...
        if (token.isReservedWord("uses")) {
            ...
            return [this._parseSubprogramDeclaration(symbolTable, NodeTypes.PROCEDURE)];
        } else if (token.isReservedWord("function")) {
            return [this._parseSubprogramDeclaration(symbolTable, NodeTypes.FUNCTION)];
        } else if (token.tokenType === TokenTypes.EOF) {
    ...
    private _parseUsesDeclaration(symbolTable) {
        ...
        do {
            ...
            var node = new Node(NodeTypes.USES, usesToken, {
                name: new Node(NodeTypes.IDENTIFIER, token)
            });
    ...
    private _parseVarDeclaration(symbolTable) {
        var nodes = [];
        ...
        do {
            ...
            do {
                ...
                var node = new Node(NodeTypes.VAR, null, {
                    name: new Node(NodeTypes.IDENTIFIER, nameToken)
                });
                nodes.push(node);
            } while (this._moreToCome(",", ":"));
            ...
            for (var i = startNode; i < nodes.length; i++) {
                ...
                nodes[i].symbol = symbolTable.addSymbol(
                    nodes[i].name.token.value, NodeTypes.VAR, type);
            }
        ...
        } while (this.lexer.peek().tokenType === TokenTypes.IDENTIFIER);
    ...
    private _parseConstDeclaration(symbolTable) {
        var nodes = [];

        do {
            // Parse the constant name.
            var token = this._expectIdentifier("expected constant name");
            var identifierNode = new Node(NodeTypes.IDENTIFIER, token);
            ...
            if (type === null) {
                ...
                node = new Node(NodeTypes.CONST, null, {
                    name: identifierNode,
                ...
                if (type.nodeType === NodeTypes.ARRAY_TYPE) {
                    rawData = this._parseArrayConstant(symbolTable, type);
                } else if (type.nodeType === NodeTypes.RECORD_TYPE) {
                    throw new PascalError(token, "constant records not supported");
                } else if (type.nodeType === NodeTypes.SIMPLE_TYPE) {
                    rawData = new RawData();
                ...
                node = new Node(NodeTypes.TYPED_CONST, null, {
                    name: identifierNode,
                    type: type,
                    rawData: rawData
                });
    ...
    private _parseArrayConstant(symbolTable, type) {
    ...
    // array of nodes.
    private _parseTypeDeclaration(symbolTable) {
        var nodes = [];
        ...
        do {
            ...
            // Parse type.
            let type = this._parseType(symbolTable, incompleteTypes);

            // Create the node.
            let node = new Node(NodeTypes.TYPE, equalToken, {
                name: identifierNode,
                type: type,
            });
        ...
        for (var i = 0; i < incompleteTypes.length; i++) {
            let node: any = incompleteTypes[i];
    ...
    private _parseSubprogramDeclaration(symbolTable, nodeType) {
        ...
        if (token.isSymbol("(")) {
            ...
            do {
                ...
                do {
                    ...
                    parameters.push(new Node(NodeTypes.PARAMETER, colon, {
                        name: new Node(NodeTypes.IDENTIFIER, token),
                ...
                let type = this._parseType(symbolTable);
        ...
        for (var i = 0; i < parameters.length; i++) {
            ...
            var symbol = symbolTable.addSymbol(parameter.name.token.value, NodeTypes.PARAMETER,
                parameter.type, parameter.byReference);
        }

        // Parse the return type if it's a function.
        var returnType;
        if (nodeType === NodeTypes.FUNCTION) {
            this._expectSymbol(":");
        ...
        let type = new Node(NodeTypes.SUBPROGRAM_TYPE, procedureToken, {
        ...
        var symbol = symbolTable.parentSymbolTable.addSymbol(nameToken.value,
            NodeTypes.SUBPROGRAM_TYPE, type);
        ...
        // Semicolon terminator.
        this._expectSymbol(nodeType === NodeTypes.PROGRAM ? "." : ";");
    ...
    private _parseBlock(symbolTable, startWord, endWord) {
    ...
    private _parseStatement(symbolTable) {
        ...
        if (token.isReservedWord("if")) {
            ...
            } else if (node.nodeType === NodeTypes.IDENTIFIER) {
    ...
    private _parseVariable(symbolTable) {
        ...
        var node = new Node(NodeTypes.IDENTIFIER, identifierToken);
        ...
        while (true) {
            ...
            if (nextToken.isSymbol("[")) {
                ...
                node = new Node(NodeTypes.DEREFERENCE, nextToken, {
                    variable: node
                });
    ...
    private _parseAssignment(symbolTable, variable) {
        var assignToken = this._expectSymbol(":=");

        var expression = this._parseExpression(symbolTable);
        return new Node(NodeTypes.ASSIGNMENT, assignToken, {
            lhs: variable,
    ...
    private _parseProcedureCall(symbolTable, identifier) {
        ...
        if (symbol.type.nodeType === NodeTypes.SUBPROGRAM_TYPE && symbol.type.returnType.isVoidType()) {
            ...
            return new Node(NodeTypes.PROCEDURE_CALL, identifier.token, {
                name: identifier,
    ...
    private _parseArguments(symbolTable, type) {
        var argumentList = [];
    ...
    private _parseIfStatement(symbolTable) {
        ...
        return new Node(NodeTypes.IF, token, {
    ...
    private _parseWhileStatement(symbolTable) {
        ...
        // Create the node.
        return new Node(NodeTypes.WHILE, whileToken, {
            expression: expression,
            statement: statement
        });
    }
    ...
    private _parseRepeatStatement(symbolTable) {
        var block = this._parseBlock(symbolTable, "repeat", "until");
        var expression = this._parseExpression(symbolTable);
        if (!expression.expressionType.isBooleanType()) {
            throw new PascalError(/* node.token */null, "repeat condition must be a boolean");
        }

        return new Node(NodeTypes.REPEAT, block.token, {
            block: block,
            expression: expression
        });
    };
    ...
    private _parseForStatement(symbolTable) {
        ...
        var variable = new Node(NodeTypes.IDENTIFIER, loopVariableToken);
        ...
        return new Node(NodeTypes.FOR, token, {
            variable: variable,
            fromExpr: fromExpr,
            toExpr: toExpr,
            body: body,
    ...
    private _parseExitStatement(symbolTable) {
        var token = this._expectReservedWord("exit");

        return new Node(NodeTypes.EXIT, token);
    }
    ...
    private _parseType(symbolTable, incompleteTypes?) {
        var token = this.lexer.next();
        var node;
        ...
        if (token.isReservedWord("array")) {
            ...
            node = new Node(NodeTypes.ARRAY_TYPE, token, {
            ...
            node = new Node(NodeTypes.SIMPLE_TYPE, token, {
                typeCode: inst.A,
                typeName: new Node(NodeTypes.IDENTIFIER, typeNameToken),
                type: type
            });
    ...
    private _parseRecordType(symbolTable, token, incompleteTypes) {
        ...
        return new Node(NodeTypes.RECORD_TYPE, token, {
            fields: fields
        });
    }
    ...
    private _parseRecordSection = function (symbolTable, fieldToken, incompleteTypes) {
        var fields = [];

        do {
            var nameToken = this._expectIdentifier("expected field name");
            var field = new Node(NodeTypes.FIELD, fieldToken, {
                name: new Node(NodeTypes.IDENTIFIER, nameToken),
                offset: 0
            });
            fields.push(field);
    ...
    private _parseRange(symbolTable) {
        var low = this._parseExpression(symbolTable);
        var token = this._expectSymbol("..");
        var high = this._parseExpression(symbolTable);

        return new Node(NodeTypes.RANGE, token, { low: low, high: high });
    }

    // Parses an expression.
    private _parseExpression(symbolTable) {
        return this._parseRelationalExpression(symbolTable);
    }

    // Parses a relational expression.
    private _parseRelationalExpression(symbolTable) {
        var node = this._parseAdditiveExpression(symbolTable);

        while (true) {
            var token = this.lexer.peek();
            if (token.isSymbol("=")) {
                node = this._createBinaryNode(symbolTable, token, node, NodeTypes.EQUALITY,
                    this._parseAdditiveExpression).withExpressionType(Node.booleanType);
            } else if (token.isSymbol("<>")) {
                node = this._createBinaryNode(symbolTable, token, node, NodeTypes.INEQUALITY,
                    this._parseAdditiveExpression).withExpressionType(Node.booleanType);
            } else if (token.isSymbol(">")) {
                node = this._createBinaryNode(symbolTable, token, node, NodeTypes.GREATER_THAN,
                    this._parseAdditiveExpression).withExpressionType(Node.booleanType);
            } else if (token.isSymbol("<")) {
                node = this._createBinaryNode(symbolTable, token, node, NodeTypes.LESS_THAN,
                    this._parseAdditiveExpression).withExpressionType(Node.booleanType);
            } else if (token.isSymbol(">=")) {
                node = this._createBinaryNode(symbolTable, token, node,
                    NodeTypes.GREATER_THAN_OR_EQUAL_TO,
                    this._parseAdditiveExpression).withExpressionType(Node.booleanType);
            } else if (token.isSymbol("<=")) {
                node = this._createBinaryNode(symbolTable, token, node, NodeTypes.LESS_THAN_OR_EQUAL_TO,
                    this._parseAdditiveExpression).withExpressionType(Node.booleanType);
            } else {
                break;
            }
    ...
    private _parseAdditiveExpression(symbolTable) {
        var node = this._parseMultiplicativeExpression(symbolTable);

        while (true) {
            var token = this.lexer.peek();
            if (token.isSymbol("+")) {
                node = this._createBinaryNode(symbolTable, token, node, NodeTypes.ADDITION,
                    this._parseMultiplicativeExpression);
            } else if (token.isSymbol("-")) {
                node = this._createBinaryNode(symbolTable, token, node, NodeTypes.SUBTRACTION,
                    this._parseMultiplicativeExpression);
            } else if (token.isReservedWord("or")) {
                node = this._createBinaryNode(symbolTable, token, node, NodeTypes.OR,
                    this._parseMultiplicativeExpression,
                    Node.booleanType);
            } else {
                break;
    ...
    private _parseMultiplicativeExpression(symbolTable) {
        var node = this._parseUnaryExpression(symbolTable);

        while (true) {
            var token = this.lexer.peek();
            if (token.isSymbol("*")) {
                node = this._createBinaryNode(symbolTable, token, node, NodeTypes.MULTIPLICATION,
                    this._parseUnaryExpression);
            } else if (token.isSymbol("/")) {
                node = this._createBinaryNode(symbolTable, token, node, NodeTypes.DIVISION,
                    this._parseUnaryExpression, Node.realType);
            } else if (token.isReservedWord("div")) {
                node = this._createBinaryNode(symbolTable, token, node, NodeTypes.INTEGER_DIVISION,
                    this._parseUnaryExpression, Node.integerType);
            } else if (token.isReservedWord("mod")) {
                node = this._createBinaryNode(symbolTable, token, node, NodeTypes.MOD,
                    this._parseUnaryExpression, Node.integerType);
            } else if (token.isReservedWord("and")) {
                node = this._createBinaryNode(symbolTable, token, node, NodeTypes.AND,
    ...
    private _parseUnaryExpression(symbolTable) {
        var node;
        ...
        if (token.isSymbol("-")) {
            ...
            node = new Node(NodeTypes.NEGATIVE, token, {
                expression: expression
            ...
            node = new Node(NodeTypes.NOT, token, {
                expression: expression
            }).withExpressionTypeFrom(expression);
        } else {
    ...
    private _parsePrimaryExpression(symbolTable) {
        var token = this.lexer.peek();
        var node;
        ...
        if (token.tokenType === TokenTypes.NUMBER) {
            ...
            node = new Node(NodeTypes.NUMBER, token);
            ...
            node.expressionType = new Node(NodeTypes.SIMPLE_TYPE, token, {
                typeCode: typeCode
            });
        } else if (token.tokenType === TokenTypes.STRING) {
            // String or character literal.
            token = this.lexer.next();
            node = new Node(NodeTypes.STRING, token);
            ...
            if (node.nodeType === NodeTypes.IDENTIFIER) {
                ...
                if (nextToken.isSymbol("(")) {
                    ...
                    symbolLookup = symbolTable.getSymbol(node.token, NodeTypes.SUBPROGRAM_TYPE);
                } else {
                ...
                if (symbol.type.nodeType === NodeTypes.SUBPROGRAM_TYPE) {
                    ...
                    node = new Node(NodeTypes.FUNCTION_CALL, node.token, {
                        name: node,
                    ...
                    if (symbol.name.toLowerCase() === "abs" &&
                        ...
                        node.argumentList[0].nodeType === NodeTypes.CAST) {
            ...
            node = new Node(NodeTypes.ADDRESS_OF, token, {
                variable: variable
            });
            node.expressionType = new Node(NodeTypes.SIMPLE_TYPE, token, {
                typeCode: inst.A,
    ...
    private _parseArrayDereference(symbolTable, variable) {
        // Make sure the variable is an array.
        if (variable.expressionType.nodeType !== NodeTypes.ARRAY_TYPE) {
        ...
        var array = new Node(NodeTypes.ARRAY, arrayToken, {
            variable: variable,
            indices: indices
        });
    ...
    private _parseRecordDesignator(symbolTable, variable) {
        // Make sure the variable so far is a record.
        var recordType = variable.expressionType;
        if (recordType.nodeType !== NodeTypes.RECORD_TYPE) {
            throw new PascalError(/* nextToken */null, "expected a record type");
        }
        ...
        // Create the new node.
        var node = new Node(NodeTypes.FIELD_DESIGNATOR, dotToken, {
            variable: variable,
            field: field
        });
    ...
    private _createBinaryNode(symbolTable, token, node,
        nodeType, rhsFn, forceType?) {

        // It must be next, we've only peeked at it.
        if (token.tokenType === TokenTypes.SYMBOL) {
    ...
    private _getCompatibleType(token, type1, type2) {
        ...
        if (type1.nodeType === NodeTypes.SIMPLE_TYPE &&
