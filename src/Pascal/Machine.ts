...
export class Machine {
    ...
    pendingDelay: number;
    control: any;
    public constructor(bytecode, keyboard?) {
    ...
    public static STATE_STOPPED = 0;
    public static STATE_RUNNING = 1;
    public static STATE_SUSPENDED = 2;
    ...
    public setDebugCallback(debugCallback) {
    ...
    private _checkDataAddress(address) {
    ...
    private _executeInstruction() {
        ...
        switch (opcode) {
            ...
            case inst.ENT:
                ...
                let address = this.mp + operand2;
                if (operand1 === 0) {
                    // Clear the local variable area.
                    for (let i = this.sp; i < address; i++) {
                        this.dstore[i] = 0;
                    }
                    this.sp = address;
                } else {
            ...
            case inst.LDA:
                ...
                address = this._computeAddress(operand1, operand2);
                this._push(address);
                break;
            case inst.LDC:
                // Load Constant.
    ...
    private _computeAddress(level, offset) {
        var mp = this.mp;
